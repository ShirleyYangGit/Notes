...
远程调用执行，producer发一条指令到broker rpc_queue队列，consumer从broker rpc_queue队列拿到指令去执行，然后将结果通过广播模式返回到broker，然后producer再从broker rpc_res队列中拿到结果
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU1MjAzMTQzMSwtMjEwMjMwNDU5NCwxNj
QwNTEyMTI5XX0=
-->